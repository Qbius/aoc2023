...
fn find_mirror(lns: Vec<String>) -> Option<(usize, usize)> {
    ...
    lns[1..].iter().enumerate().rev().find_map(|(mirror_i_minus_one, rev_str)| {
        ...
        let reflection_length = mirror_i / 2;
...
fn first(rowsandcolumns: Vec<(Vec<String>, Vec<String>)>) -> usize {
    rowsandcolumns.into_iter().filter_map(|(rows, cols)| {
        let row_count = rows.len();
        let col_count = cols.len();
        let reverse_rows: Vec<String> = rows.iter().cloned().rev().collect();
        let reverse_cols: Vec<String> = cols.iter().cloned().rev().collect();
        let row_mirror = find_mirror(rows).or_else(|| find_mirror(reverse_rows).map(|(i, len)| (row_count - i - 1, len))).map(|i| (Dir::Row, i));
        let col_mirror = find_mirror(cols).or_else(|| find_mirror(reverse_cols).map(|(i, len)| (col_count - i - 1, len))).map(|i| (Dir::Column, i));
        row_mirror.or(col_mirror)
